...
contract DeftPair is IDeftPair, DeftLPT {
    ...
    modifier noReentrant() {
        require(unlocked == 1, "DeftPair: RE-ENTRANT");
        unlocked = 0;
        _;
        unlocked = 1;
    }
    ...
    modifier onlyRouter() {
        require(msg.sender == ROUTER, "DeftPair: Must be called from Router");
        _;
    }
    ...
    function mint(address to) external noReentrant returns (uint256 liquidity) {
        ...
        require(liquidity > 0, "DeftPair: INSUFFICIENT_LIQUIDITY_MINTED");
        _mint(to, liquidity);
    ...
    function swap(
        ...
        address to,
        bytes calldata data
    ) external noReentrant onlyRouter {
        require(
            amount0Out > 0 || amount1Out > 0,
            "DeftPair: INSUFFICIENT_OUTPUT_AMOUNT"
        );
        (uint112 _reserve0, uint112 _reserve1, ) = getReserves(); // gas savings
        require(
            amount0Out < _reserve0 && amount1Out < _reserve1,
            "DeftPair: INSUFFICIENT_LIQUIDITY"
        );
        ...
        require(
            ...
            "DeftPair: INSUFFICIENT_INPUT_AMOUNT"
        );
        {
    ...
    function sync() external noReentrant {
        _update(
        ...
        unchecked {
            ...
            if (timeElapsed > 0 && _reserve0 != 0 && _reserve1 != 0) {
                ...
                price0CumulativeLast +=
                    uint256((_reserve1.encode()).uqdiv(_reserve0)) *
                    timeElapsed;
                price1CumulativeLast +=
                    uint256((_reserve0.encode()).uqdiv(_reserve1)) *
                    timeElapsed;
            }
        }
    ...
    function _mintFee(uint112 _reserve0, uint112 _reserve1)
        ...
        returns (bool feeOn)
    {
        address feeTo = IDeftFactory(FACTORY).feeTo();
